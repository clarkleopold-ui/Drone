...
def parse_ack(line):
    ...
    # Add/remove servo keys here depending on what the Arduino actually prints.
    for key in ["motor_pwm", "servo1", "servo2", "servo3", "servo4", "lx", "ly", "rx", "ry"]:
...
class DroneApp(tk.Tk):
    def __init__(self):
        ...
        self.motor_speed_var = tk.StringVar(value="0%")
        self.direction_var = tk.StringVar(value="0 Stopped")
    ...
    def build_ui(self):
        ...
        # Live status frame
        status_box = ttk.LabelFrame(root, text="Live Status", padding=10)
        status_box.pack(fill="x", pady=15)

        for i in range(5):
            status_box.columnconfigure(i, weight=1)

        self.motor_bar = ttk.Progressbar(status_box, maximum=100)
        self.servo1_bar = ttk.Progressbar(status_box, maximum=180)
        self.servo2_bar = ttk.Progressbar(status_box, maximum=180)
        self.servo3_bar = ttk.Progressbar(status_box, maximum=180)
        self.servo4_bar = ttk.Progressbar(status_box, maximum=180)

        self.make_metric(status_box, "Motor Speed", self.motor_speed_var, 0, 0)
        self.motor_bar.grid(row=2, column=0, sticky="ew", padx=10)

        self.make_metric(status_box, "Servo 1", self.servo1_var, 0, 1)
        self.servo1_bar.grid(row=2, column=1, sticky="ew", padx=10)

        self.make_metric(status_box, "Servo 2", self.servo2_var, 0, 2)
        self.servo2_bar.grid(row=2, column=2, sticky="ew", padx=10)

        self.make_metric(status_box, "Servo 3", self.servo3_var, 0, 3)
        self.servo3_bar.grid(row=2, column=3, sticky="ew", padx=10)

        self.make_metric(status_box, "Servo 4", self.servo4_var, 0, 4)
        self.servo4_bar.grid(row=2, column=4, sticky="ew", padx=10)

        ttk.Label(status_box, text="Direction", font=("Times New Roman", 11, "bold")).grid(
            row=3, column=0, pady=(15, 0)
        )
        ttk.Label(status_box, textvariable=self.direction_var, font=("Times New Roman", 14)).grid(
            row=4, column=0
        )
    ...
    def update_dashboard(self, ack_line):
        ...
        self.motor_speed_var.set(f"{motor_percent}%")
        self.motor_bar["value"] = motor_percent
