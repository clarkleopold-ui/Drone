...
/*
  ...
  WAV channel 2: ACS72981LLRATR-050B3 current sensor

  The logger is intentionally separate from the ESP32 flight controller.
...
// The ACS72981LLRATR-050B3 is 26.4 mV/A at 3.3 V. This is about
// 32.76 ADC counts per amp at 12-bit resolution.
constexpr float ACS72981_MV_PER_AMP_AT_3V3 = 26.4f;
constexpr float CURRENT_COUNTS_PER_AMP =
    ADC_MAX_VALUE * (ACS72981_MV_PER_AMP_AT_3V3 / 1000.0f) / 3.3f;
...
bool calibrateCurrentZero() {
  ...
  if (!currentZeroValid) {
    Serial.println(
        "ERROR Zero value is outside the expected ACS72981 range. "
...
bool writeMetadataFile(uint32_t dataBytes) {
  ...
  metadata.print(ADC_MIDPOINT);
  metadata.println(",");
  metadata.print("  \"current_sensor\": \"ACS72981LLRATR-050B3\",");
  metadata.println();
...
void printHelp() {
  ...
  Serial.println("  zero   - calibrate ACS72981 with every motor off");
