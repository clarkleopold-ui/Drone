...
DroneState::DroneState() {  //Constructor
  ...
  rx = 0;
  ry = 0;
  throttlePercent = pitch = roll = yaw = 0;
...
void DroneState::state_circle_motor() {
  ...
  if (cross || !r1) { 
    ...
    throttlePercent = (THROTTLE_MIN/THROTTLE_MAX)*100;
    pitchOffSet = rollOffSet = yawOffSet = 0;
  } else if (free_move){
    ...
    pitch = constrain(ry, -100, 100);
    roll = constrain(rx, -100, 100);
    yaw = constrain(lx, -100, 100);
    ...
    if (ly >= 0) {
      ...
      throttle = map(ly, -100, 0, THROTTLE_MIN, HOVER_THROTTLE);
    } else if (ly <= 0) {
      throttle = map(ly, 0, 100, HOVER_THROTTLE, THROTTLE_MAX);
    }
    delta = throttle * 0.15;
    //When moving the right joystick in the y-axis it will tilt the drone forward and back (pitch) 
    if (ry) pitchOffSet = map(pitch, -100, 100, -delta, delta);
    //When moving the right joystick in the x-axis it will tilt the drone left and right (roll)
    if (rx) rollOffSet = map(roll, -100, 100, -delta, delta);
    ...
    if(lx) yawOffSet = map(yaw, -100, 100, -delta, delta);
  }
  ...
  roll = constrain(roll, THROTTLE_MIN, THROTTLE_MAX);
  pitch = constrain(pitch, THROTTLE_MIN, THROTTLE_MAX);
  yaw = constrain(yaw, THROTTLE_MIN, THROTTLE_MAX);
  //Calculate the pitch, roll, and yaw, pwm values based on the gyroscope and accelerometer
  int pitch_gyro_accel = map(KalmanAnglePitch, -60, 60, -delta, delta);
  int roll_gyro_accel = map(KalmanAngleRoll, -60, 60, -delta, delta);
  motorPwm[0] = throttle + pitch_gyro_accel + roll_gyro_accel - pitchOffSet + rollOffSet + yawOffSet;
  motorPwm[1] = throttle - pitch_gyro_accel + roll_gyro_accel + pitchOffSet + rollOffSet - yawOffSet;
  motorPwm[2] = throttle + pitch_gyro_accel - roll_gyro_accel - pitchOffSet - rollOffSet - yawOffSet;
  motorPwm[3] = throttle - pitch_gyro_accel - roll_gyro_accel + pitchOffSet - rollOffSet + yawOffSet;
  for (int y=0; y<4; y++)
