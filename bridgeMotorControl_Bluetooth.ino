...
// ---------- USB + ESP32 Bluetooth serial bridge ----------
// USB Serial keeps the existing wired path. Serial1 connects to the ESP32.
// Both links use the same newline-terminated JSON commands and ACK telemetry.
...
class ControllerClass {
  public:
    ...
    void displayValues_and_buttonPress(Print& output) const;
...
// Read one complete newline-terminated command from either USB Serial or
// Serial1. Each transport has its own buffer so partial lines cannot mix.
  ...
  while (input.available() > 0) {
    ...
    if (c == '\n') {
      if (!state.overflow && state.index > 0) {
        ...
        controller.parseControllerLine(state.buffer);
      }

      state.index = 0;
      state.overflow = false;
    } else if (c != '\r' && !state.overflow) {
...
void ControllerClass::displayValues_and_buttonPress(Print& output) const{
  // ---------- Debug output ----------
  ...
  /*output.print(" servo1=");
  output.print(data.servoAngle[0]);
  output.print(" servo2=");
  output.print(data.servoAngle[1]);
  output.print(" servo3=");
  output.print(data.servoAngle[2]);
  output.print(" servo4=");
  output.print(data.servoAngle[3]);*/
  output.print(" lx=");
  ...
  output.print(" Roll [°]: ");
  output.print(data.KalmanAngleRoll);
  output.print(" Pitch [°]: ");
  output.print(data.KalmanAnglePitch);
  output.print(" Yaw [°]: ");
  output.print(data.AngleYaw);
  output.print(" Yaw Rate [°/s]: ");
...
void setup() {
  ...
  Serial1.println("Arduino bridge ready");
}

// ---------- Main loop ----------
void loop() {
  // Accept the same command protocol from wired USB and the ESP32.
  processSerialInput(Serial, usbRx);
  processSerialInput(Serial1, bluetoothRx);
  ...
  // Send the identical ACK to wired USB and the ESP32 Bluetooth transport.
  if (millis() - lastTelemetryMs >= TELEMETRY_PERIOD_MS) {
    lastTelemetryMs = millis();
    controller.displayValues_and_buttonPress(Serial);
    controller.displayValues_and_buttonPress(Serial1);
  }
}
